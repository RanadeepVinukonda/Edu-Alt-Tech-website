    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          ...
          <button onClick={onClose} className="p-2 hover:bg-slate-200 rounded-xl transition-colors">✕</button>
        </div>
...
const TeacherDashboard: React.FC<Props> = ({ user, activeTab }) => {
  ...
  return (
    <div className="animate-in fade-in duration-500">
      {activeTab === 'overview' && (
        <div>
          <h2 className="text-3xl font-bold text-slate-900 mb-6">Teacher Overview</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-6">
              <div className="w-14 h-14 bg-emerald-50 rounded-2xl flex items-center justify-center flex-shrink-0 text-emerald-600">
        ...
        <div>
          <div className="flex justify-between items-center mb-6">
            ...
            <button 
              ...
              className="btn-shine px-5 py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition-colors flex items-center gap-2"
            >
          ...
          {courses.length === 0 ? (
            <div className="py-20 text-center flex flex-col items-center bg-white rounded-3xl border border-slate-100 shadow-sm">
              ...
              <button onClick={() => setIsModalOpen(true)} className="btn-ripple px-6 py-3 bg-emerald-50 text-emerald-700 font-bold rounded-xl hover:bg-emerald-100 transition-colors">
                Create First Course
              </button>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {courses.map(course => (
                <div key={course.id} className="card-hover group p-6 bg-white rounded-3xl border border-slate-200 hover:border-emerald-200 hover:shadow-lg transition-all flex flex-col gap-4">
                  <div className="flex justify-between items-start">
                    <div className="p-3 bg-slate-50 rounded-2xl text-slate-400 group-hover:text-emerald-500 transition-colors">
                      <BookOpen className="w-6 h-6" />
                    </div>
                    <button onClick={() => handleDeleteCourse(course.id)} className="p-2 opacity-0 group-hover:opacity-100 hover:bg-red-50 hover:text-red-600 rounded-lg transition-all bg-white border border-slate-100 hover:border-red-100 shadow-sm">
                  ...
                  <div className="flex items-center gap-2">
                    {editingPriceCourseId === course.id ? (
                      <>
                        <div className="flex items-center gap-1 flex-1 bg-slate-50 border border-emerald-300 rounded-xl px-3 py-2 focus-within:ring-2 focus-within:ring-emerald-100">
                          <IndianRupee className="w-4 h-4 text-slate-400 flex-shrink-0" />
                          <input
                            type="number" min="0" step="1" autoFocus
                            className="bg-transparent outline-none w-full text-sm font-bold text-slate-900"
                            placeholder="0 = Free"
                        ...
                        <button onClick={() => setEditingPriceCourseId(null)} className="px-3 py-2 bg-slate-100 text-slate-600 text-xs font-bold rounded-xl hover:bg-slate-200 transition-colors">Cancel</button>
                      </>
                    ) : (
                      <button
                        onClick={() => { setEditingPriceCourseId(course.id); setEditingPriceValue(course.price != null ? String(course.price) : ''); }}
                        className="flex items-center gap-1.5 px-3 py-2 bg-slate-50 hover:bg-emerald-50 hover:text-emerald-700 text-slate-600 rounded-xl text-sm font-bold transition-colors border border-slate-100 hover:border-emerald-200"
                      >
                  ...
                  <div className="mt-auto pt-4 flex gap-2 w-full border-t border-slate-100">
                    <button onClick={() => setSelectedCourse(course)} className="btn-ripple flex-1 py-2 bg-slate-50 text-slate-600 hover:bg-slate-100 hover:text-slate-900 font-bold text-sm rounded-lg transition-colors text-center w-full">
                      Manage Structure
                    </button>
                  </div>
                </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Create New Course">
        <form onSubmit={handleCreateCourse} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-emerald-400 focus:ring-4 focus:ring-emerald-50 transition-all font-medium h-32 resize-none"
          ...
          <div>
            ...
            <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-2xl px-5 focus-within:border-emerald-400 focus-within:ring-4 focus-within:ring-emerald-50 transition-all">
              <IndianRupee className="w-4 h-4 text-slate-400 flex-shrink-0" />
              <input
