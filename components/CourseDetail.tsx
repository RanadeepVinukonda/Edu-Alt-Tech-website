...
const CourseDetail: React.FC<Props> = ({ course, onBack }) => {
      ...
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
        <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
          <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            ...
            <button type="button" onClick={onClose} className="p-2 hover:bg-slate-200 rounded-xl transition-colors">✕</button>
          </div>
          <div className="p-6">
            <form onSubmit={onSubmit} className="space-y-4">
              ...
              <button
                ...
                className="w-full py-4 bg-slate-900 text-white font-bold rounded-2xl flex items-center justify-center gap-2 hover:bg-slate-800 transition-all disabled:opacity-50 mt-4 shadow-lg"
              >
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <button onClick={onBack} className="flex items-center gap-2 text-slate-500 hover:text-slate-900 font-bold mb-6 transition-colors">
        <ArrowLeft className="w-4 h-4" /> Back to Courses
      </button>

      <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm mb-8">
        <h2 className="text-3xl font-bold text-slate-900 mb-2 truncate">{course.title}</h2>
        <p className="text-slate-500 leading-relaxed">{course.description}</p>
      </div>

      <div className="flex justify-between items-center mb-6">
        <h3 className="text-2xl font-bold text-slate-900">Syllabus Structure</h3>
        <button 
      ...
      {subjects.length === 0 ? (
        <div className="py-20 text-center flex flex-col items-center bg-white rounded-3xl border border-slate-100 shadow-sm">
          ...
          <h3 className="text-xl font-bold text-slate-900 mb-2">No subjects added</h3>
          <p className="text-slate-400 max-w-sm">Create subjects like "Physics" or "Module 1" to start organizing this course.</p>
        </div>
      ) : (
        <div className="space-y-6">
          {subjects.map(subject => (
            <div key={subject.id} className="bg-white rounded-[2rem] border border-slate-200 overflow-hidden shadow-sm">
              <div className="p-6 bg-slate-50 border-b border-slate-200 flex justify-between items-center group">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-white rounded-xl shadow-sm text-emerald-600 flex items-center justify-center font-bold">
                    <FolderOpen className="w-5 h-5 fill-current opacity-20" />
                  </div>
                  <h4 className="text-xl font-bold text-slate-900">{subject.title}</h4>
                </div>
                <div className="flex items-center gap-2">
                  <button 
                    onClick={() => { setActiveSubjectId(subject.id); setIsChapterModalOpen(true); }}
                    className="px-4 py-2 bg-white text-slate-700 font-bold text-sm rounded-lg hover:bg-slate-100 transition-colors border border-slate-200 shadow-sm"
                  >
                    + Add Chapter
                  </button>
                  <button onClick={() => handleDeleteSubject(subject.id)} className="p-2 opacity-0 group-hover:opacity-100 hover:bg-red-50 hover:text-red-600 rounded-lg transition-all text-slate-400">
                ...
                {(chapters[subject.id] || []).length === 0 ? (
                  ...
                  <ul className="space-y-3">
                    {chapters[subject.id].map(chapter => (
                      <li key={chapter.id} className="flex justify-between items-center p-4 bg-slate-50/50 hover:bg-slate-50 rounded-2xl group transition-colors border border-transparent hover:border-slate-100">
                        <div className="flex items-center gap-3">
                          <FileText className="w-5 h-5 text-slate-400" />
                          <span className="font-bold text-slate-700">{chapter.title}</span>
                        </div>
                        <button onClick={() => handleDeleteChapter(subject.id, chapter.id)} className="p-2 opacity-0 group-hover:opacity-100 hover:bg-red-50 hover:text-red-600 rounded-lg transition-all text-slate-400">
